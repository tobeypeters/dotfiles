...
# Added : Tobey Peters : start : https://github.com/tobeypeters/dotfiles
#declare -a forbidden_names=('Audacious Playlist Editor' 'Alacritty')
declare -a forbidden_names=('Audacious Playlist Editor')
# Added : Tobey Peters : end
...
# Added : Tobey Peters : start
		# Don't show the window if its name is forbidden
		declare -l cmpa
		declare -l cmpb
		...
		do
			cmpa=${forbidden_names[i]}
			cmpb=$title

			[[ $cmpa == $cmpb ]] && forbidden=true
		done
		[[ $forbidden == true ]] && continue
# Added : Tobey Peters : end
